   ...
   */
    ...
    */
	function _createFrame(secureUri){
		...
		io.style.position = 'absolutes YAH/ ifrssiti-1000pxsolutes YAH/ ileftsiti-1000pxsol
v');

		document.body.appendChild&');

		YAHOO.log('File upload i] + ' crroyed. Id is:' + frameId, 'info', 'ConnectiData;
	}

   /**
    * @descriP	_fnrame a// If POSTfaulttion Crhiddener the form elandler
    * forkey-abel, vable,body.rame mdition to the HTMLlback object.
  This mebody.Psend(poeFrame
    * @private
    * @static
    * @param {st.send(posvalue The// If POSin IE.
    * @rearrayobjectEform eleCents collef(thiddeneratic fivoid}
    */
	funcebody.Psend(pon.send(po)Uri)
	{
jectEform ele	dataameI.senMPOST metData = poderStr.sjoiem = 0,eader, delimit
					  for( < .senMPOST mheader.length; ak;
	    delimit.senMPOST mheader[i].ind=iframe')     if(delimitPos !=an')jectEform elhea
			io = document.createElem=== 't.text)jectEform elhea& (objitihidden'.text)jectEform elhea&			io.next = decodeURIComp.senMPOST mheader[i].substring(0,delimt.text)jectEform elhea&v				oVaext = decodeURIComp.senMPOST mheader[i].substring(delim1mt.text)se;
		this._fot.body.appendjectEform elheagument);
			on');
   ...
   /**
    * @descriisFile Resets HTM,icallysivnt-Ty fors/		/ form elP pailitati   /**
pload i] + ' cr(procall createFraframe factialize the transobject.
  This m'File .logeFrame
    * @private
    * @static
    * @param idbject} The transai method.
    * @param {object} calback  User-defined callback objecic
    * @param {st="' Fis a fully qualified paframe reobjecic
    * @param {st.send(pos// If POSTframe a are subeptiodic condponseon, HTML n IE.
    * @return {void}
    */
	func'File .logsponse(o, callthod,.send(po)Uri)      //pload ihument i uplUsext typId = "// The uppor   / set buniquct} The transai mer IE.
		var frameId = 'yuibject(
ct>',

	YCM.um-urilitameelect-art/x-www POSbject>',
  var io = document.getElementById(frject>',
 e8			ie9 >= 8 === 9) ? true : fal>',
  var oConn = ent      args = (callback && callback.argument)?callback.argument:null,
      			if(oEs,i,eaderlba, rawhis.Aname attr,

	YCM.Cent)?ca;eUri){
ptionL figi opto the HTML name attributelemenrawhis.Aname attr.tId ent trans:se;
		this._fot  obname att(' transaiem =  This :se;
		this._fot  obname att(' This aiem =  form :se;
		this._fot  obname att(' form 't.ty};eUri){
Ially initializesets HTML form propeiceptionaliyertieri){
 doe User-deponse in on, HTML f)se;
		this._fots obname att(' transalltho) false;
		this._fots obname att(' This a,() === ) false;
		this._fots obname att(' form ', tById(frame	io;

		if(YAHOO.env.ua.ie 8bject'){

		// IE doe    rm o.
	* @prorm- (objf form or HTMsm name.
Ient" ini objed calls
	* @pro- "em-urili"o true.
			this._fots obname att('em-urili',

	YCM.um-uriliormId]);
		}
{ true.
			this._fots obname att('em-ttent-T
	YCM.um-uriliormId])e	io;
.send(po)Uri)i){
			oEstem++] =ebody.Psend(pon.send(po)return;
		}
		_xdon for file uploe.
			this._fots are mState( {
			// Fire global custom event -- startEv      this.startEvent.fire(o, art);

			if(o.startEvent{
				// Fire transaction custom event -- startEve      o.startEvent.fire(o, aturn;
		}
		_xdor the polfd from a calisns are pronse, and thr tibject
		// pyihumes not bs undefin      if(callback && callback.timetrue.
			this._timeOut[o.tId] = window.setTimeout(function(){ oConn.abort(o, callback, true); }, callback.timeturn;
		}
   // Rrm or form elem + ' crurnebody.Psend(poe:
						if(oEs.valu		if(oEsheader. > 0imetru				  for( < u		if(oEsheader.length; ak;oe.
			this._fot.body.removeoEform elheagument);
			on'	}
  ionreto the HTML name attsdition lue figi op
if
					//siframe pron for file uplo      for(prrawhis.Aname attr& !ie9)     if(YAHOO.lang.hasOwnProrawhis.Aname attr,
ders, pr){
				rawhis.Aname attreaders[Value){e.
			this._fots obname att(eader(rawhis.Aname attreaders[oValue;
			
		}
{ tru;oe.
			this._fot.body.bname att(eade[oValue;
			);
			on'	}
  ietto the HTMLnread * its prope
opt;

		this.resetFormState(      // Creat'File u   if(calponse h event that fires// of the te(   received callile uhange ev   /LHttntlyessfulustom ponse h usagetnot r
alize
	destroy the i.body.dt sent fro io = doc
al
	YCM.Cent)?castener = function( IE.
umen,
de ifrponse!ie9)     if(callback && callback.timetru	      window.clearTimeout(oConn._timeOut[o.teak;
	    delete oConn._timeOut[o.	
			on'( {
			// Fire global custom event -- completeEve      oConn.completeEvent.fire(o, ar	t);

		 if(o.completeEv	ent{
				// Fire transaction custom event -- completeEve	      o.completeEvent.fire(o, a	
			on'(      oon'(    ' + :ibject(
ct(    / The arg:ire(o:null,
      };eUri	    	pe)
				umen   i	}

	er CW     w io = document.getEsByTag+] =('umen')[0ions[j]de    i	}

	er CW     w io = document.getEsByTag+] =('p:fai[0ionr){
			 (umenlected) {		 (p:flected) {
tponseTp:f : opeader C?p:f : opeader C:p:f ;
		csponserue;
						}		}
	cted) {
tponseTument: opeader C?ument: opeader C:ument;
		csponserue;
												      obj.responseTtponserue;      // responslable obj.responlback wipopul ' cr    *ce in the POST sent froy the .erue;   Sase, sinefault HTTP hection cann.conT sent froy the 					      obj.responsei	}

	er CW     w io = docspoDio = do?i	}

	er CW     w io = docspoDio = do:i	}

	er CW     w io = doument);
		      catch(ie9)     if(callback && cal if(o.upl){
				   if(!callback.scrue;
ck && cal if(o.ponet.text);e{
			YAHOO.File u   if(caaction.', 'info', 'Connectialue;
			
		}
{ tru;ock && cal if(o.ailure.apply(callback.scone]t.text);e{
			YAHOO.File u   if(callback with ction.', 'info', 'Connectialue;
					on'( {
			// Fire global custom eve
	YCM.uploaeEve      
	YCM.uploaeEvent.net.tr	t);

		
	YCM.uploaeEv	ent{
				// Fire transaction custom eve
	YCM.uploaeEve	o 
	YCM.uploaeEvent.net.t					on'(YE.Event.removeListener(io, ,

	YCM.Cent)?cat.tr	t)indow.setTitext)j */
(functioeak;
     document.body.removeChildoeak;       this.releaseObjecext);e{
			YAHOO.log('File upload iframe destroyed. Id is:' + frameId, 'info', 'Connectialue;, 100imeturtate(   

	// Binonile uponse h eoestroy the iorm() tonstru* with file ue xdr respYE){
			YE.addListener(io, ,

	YCM.Cent)?cat.t			onxdr;
rue, se=nction _s;onxdr;	this.resetFore=nc	this.resetFor;onxdr;ocall create=nction _creat;onxdr;ebody.Psend(poe=ncebody.Psend(po;    */
	YCM..log(=nc'File .logsponse;
}e{
			regE.adr("RI to conn ,
YCM = YAHOO.util.Con{vP hans: "2.9.0 ,
on to: "2800"}
