$(document).ready(function() {
	...
	var sixPos = $("#section6").position();
	var sevenPos = $("#section7").position();
	
	//nav function
	//subtract 172 because that is the height of fixed postion header
	 $('ul.nav a, .sup').bind('click',function(event){
		killnav = true;
		var $anchor = $(this);
...
function scrollcheck() {
	...
	if (killnav == false) {
	  ...
	  //set nav
	  if ($(this).scrollTop() > (sixPos.top-172)) { 
	  $('ul.nav a').removeClass("active");
	  $('ul.nav li:nth-child(6) a').addClass("active");
	  inproc = true;			
	  }
	  //set nav
	  if ($(this).scrollTop() > (sevenPos.top-172)) { 
	  $('ul.nav a').removeClass("active");
	  $('ul.nav li:nth-child(7) a').addClass("active");
	  inproc = true;			
	  }
	} else {
		//do nothing				
...
function sectioncheck() {
	...
	if (/#section7$/.test(window.location)) {
	    gotoSection('section7');
	}
});
...
function gotoSection(section) {
	$('ul.nav a').removeClass("active");
	$('html, body').stop().animate({
		scrollTop: $('#' + section).offset().top-170}, 800, function(){ killnav = false;  });		
		event.preventDefault();
}