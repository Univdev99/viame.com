...
class OAuth2Client
{
    ...
    */
     ...
     * Exception Codes
     */
    const EC_CURL_NOT_FOUND                     = 0x01;
    const EC_CURL_ERROR                         = 0x02;
    const EC_GRANT_TYPE_ERROR                   = 0x03;
    const EC_INVALID_CLIENT_AUTHENTICATION_TYPE = 0x04;
    const EC_INVALID_ACCESS_TOKEN_TYPE          = 0x05;
    
    /**
     * Invalid Argument Exception Codes
     */
    const IAEC_INVALID_GRANT_TYPE      = 0x01;
    const IAEC_CERTIFICATE_NOT_FOUND   = 0x02;
    const IAEC_REQUIRE_PARAMS_AS_ARRAY = 0x03;
    const IAEC_MISSING_PARAMETER       = 0x04;
    ...
    public function __construct($client_id, $client_secret, $client_auth = self::AUTH_TYPE_URI, $certificate_file = null)
    {
        if (!extension_loaded('curl')) {
            throw new Exception('The PHP exention curl must be installed to use this library.', self::EC_CURL_NOT_FOUND);
        }
        ...
        if (!empty($this->certificate_file)  && !is_file($this->certificate_file)) {
            throw new InvalidArgumentException('The certificate file was not found', self::IAEC_CERTIFICATE_NOT_FOUND);
        }
    }
    ...
    public function getAccessToken($token_endpoint, $grant_type, array $parameters)
    {
        /*
        if (!$grant_type) {
            throw new InvalidArgumentException('The grant_type is mandatory.', self::IAEC_INVALID_GRANT_TYPE);
        }
        ...
        if (!class_exists($grantTypeClass)) {
            throw new InvalidArgumentException('Unknown grant type \'' . $grant_type . '\'', self::IAEC_INVALID_GRANT_TYPE);
        }
        $grantTypeObject = new $grantTypeClass();
        $grantTypeObject->validateParameters($parameters);
        if (!defined($grantTypeClass . '::GRANT_TYPE')) {
            throw new Exception('Unknown constant GRANT_TYPE for class ' . $grantTypeClassName, self::EC_GRANT_TYPE_ERROR);
        }
        ...
        */
        ...
        switch ($this->client_auth) {
            ...
            case self::AUTH_TYPE_AUTHORIZATION_BASIC:
                ...
                break;
            default:
                throw new Exception('Unknown client auth type.', self::EC_INVALID_CLIENT_AUTHENTICATION_TYPE);
                break;
        }
        ...
        if ($this->access_token) {
            switch ($this->access_token_type) {
                case self::ACCESS_TOKEN_URI:
                    if (is_array($parameters)) {
                        ...
                        throw new InvalidArgumentException(
                            ...
                            self::IAEC_REQUIRE_PARAMS_AS_ARRAY
                        );
                    }
                    break;
                ...
                case self::ACCESS_TOKEN_MAC:
                    ...
                    break;
                default:
                    throw new Exception('Unknown access token type.', self::EC_INVALID_ACCESS_TOKEN_TYPE);
                    break;
            }
        }
    ...
    private function executeRequest($url, $parameters = array(), $http_method = self::HTTP_METHOD_GET, array $http_headers = null, $form_content_type = self::HTTP_FORM_CONTENT_TYPE_MULTIPART)
    {
        ...
        if ($curl_error = curl_error($ch)) {
            throw new Exception($curl_error, self::EC_CURL_ERROR);
        } else {
