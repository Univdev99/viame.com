...
class Blog_CreateController extends ViaMe_Controller_Default_Create
{
    ...
    {
        $this->view->headTitle('Create Blog Post', 'PREPEND');
        ...
        if ($this->getRequest()->isPost() && $form->isValid($this->_getAllParams())) {
            $fields = array('title' => 'title', 'heading' => 'heading', 'summary' => 'summary', 'more_link' => 'more_link', 'meta_title' => 'meta_title', 'meta_description' => 'meta_description', 'meta_keywords' => 'meta_keywords', 'allow_comments' => 'allow_comments', 'allow_ratings' => 'allow_ratings', 'content' => 'content', 'show_on_fail' => 'show_on_fail', 'mid' => 'matrix_counter');
            ...
            try {
                ...
                if (isset($data['active']) && $data['active'] == 'f') {
                    $this->log->ERR('_blogSaved', array('item_counter' => $last_id));
                    #$this->VMAH->log('log message', Zend_Log::EMERG, array('test1', 'test2', 'test3'));
                }
                else {
                    $this->log->ERR('_blogPublished', array('item_counter' => $last_id));
                ...
                $this->view->formErrors = array('That blog post was not successfully created.');
            }
        }
        else {
            $form->populate($_GET);
        ...
        $this->view->form = $form;
        $this->view->page_title = 'Create A New Blog Post' . ($this->target->currentModule->display ? ' (' . $this->target->currentModule->display . ')' : '');
    }
}