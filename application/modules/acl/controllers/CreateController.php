...
class Acl_CreateController extends ViaMe_Controller_Action
{   
    ...
    public function indexAction()
    {
        ...
        */
        ...
        /*
            Things that need to be done outside of any cache due to parameters
        */
            // Load Groups
            $temparray = array();
            ...
            foreach (array('mod', 'mid', 'iid') as $temp) {
            ...
            $form->addElement('Submit', 'submit', array('label' => 'Create ACL', 'ignore' => true, 'order' => 999));
            $form->addElement('Submit', 'cancel', array('label' => 'Cancel', 'order' => 1000));

        
        // If posted, validate, and write to db
        if ($this->getRequest()->isPost() && $form->isValid($this->_getAllParams())) {
            $fields = array('mod' => 'module_id', 'mid' => 'matrix_counter', 'iid' => 'item_counter', 'title' => '', 'display' => '', 'orderby' => '', 'w_registration_status' => '', 'w_dos' => '', 'w_password' => '', 'w_member_initial_amount' => '', 'w_member_trial_amount' => '', 'w_member_trial_quantity' => '', 'w_member_trial_interval' => '', 'w_member_amount' => '', 'w_member_quantity' => '', 'w_member_interval' => '', 'w_member_auto_renew' => '', 'privilege' => '', 'filter' => '', 'description' => '', 'recursive' => '', 'invisible' => '', 'w_arb_profiles' => '', 'w_arb_profiles_ex' => '');
            $params = (object) $form->getValues();
            foreach ($fields as $key => $val) {
                if (isset($params->$key) && $params->$key !== '') {
                    $data[($val ? $val : $key)] = $params->$key;
                }
            }
            ...
            foreach (array('w_groups', 'w_contact_profiles') as $key) {
                if ($this->_getParam($key)) {
                    $data[$key] = '{' . implode(',', $this->_getParam($key)) . '}';
                }
            }
            ...
            if ($this->_getParam('w_member_start')) {
                ...
                $data['w_member_start'] = $date->get(Zend_Date::ISO_8601);
            }
            ...
            try {
                ...
                $acl_acls->insert($data);
                
                $this->_autoredirect($this->target->pre . '/' . $this->getRequest()->getModuleName() . '/');
                
            } catch (Exception $e) {
                $this->view->formErrors = array('That ACL was not successfully created.');
