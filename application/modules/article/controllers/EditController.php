...
class Article_EditController extends ViaMe_Controller_Default_Edit
{
    ...
    {
        $this->view->headTitle('Edit Article', 'PREPEND');
        ...
        if ($this->getRequest()->isPost() && $form->isValid($this->_getAllParams())) {
            ...
            $where[] = $article_articles->getAdapter()->quoteInto('matrix_counter=?', $this->_getParam('mid'));
            $where[] = $article_articles->getAdapter()->quoteInto('counter=?', $this->_getParam('id'));
            
            try {
                $article_articles->update(
                    $data,
                    $where
                );
                
                $this->log->ERR('_articleEdited', array('item_counter' => $this->_getParam('id')));
                
                if ($this->_getParam('saveedit')) {
                    $this->view->formErrors = array('Article Saved.');
                }
                else {
                    $this->_autoredirect($this->target->pre . '/' . $this->getRequest()->getModuleName() . '/p/mid/' . $this->_getParam('mid') . '/');
                }
                ...
                $this->view->formErrors = array('That article was not successfully modified.');
            }
        ...
        $this->view->form = $form;
        $this->view->page_title = 'Edit Article' . ($this->_modObject->title ? ' (' . $this->_modObject->title . ')' : '');
